...
+ parsing : 가공되지 않은 문자열에서 필요한 부분을 추출하여 의미있는(구조화된) 데이터로 만드는 과정
...
상당히 과격한 작업을 수행하기 때문에 실서비스 환경에서는 사용하지 않는 것이 바람직하다. 개발환경에서 사용하는 것이 좋다.
...
+ twitter 크롤링 연습
+ basketball 사이트 크롤링 연습

### 정리
+ 위의 git 사이트 참고

### 일지
+ python crawling 강의사이트 :  https://www.youtube.com/watch?v=o0RasL5uxkg
+ 와 python 진짜 편하다. 그리고 크롤링 완전 꿀잼.! 졸작을 이쪽으로 할 걸.. 아쉽다ㅜㅜ


+ 먹어본 음식

~~~~
일본라면, 오리볶음밥, 잡채밥, 부대찌개, 제육덮밥, 피자무한리필, 햄버거, 커피겁내게 큰 싸이즈, 어떤 콩나물제육볶음같은것인데 이름이 기억이안남, 짬뽕, 회전초밥, 고기 무한리필, kfc 햄버거, 돼지찌개, 맥도날드 상하이버거, 닭갈비, 계란볶음밥 etc
